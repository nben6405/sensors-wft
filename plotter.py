...
def save_final_plots(csv_path):
    ...
    fig.suptitle('Stacked Data Plot (Brakes and Shocks)') # Add a main title for clarity
    ...
    ax1.legend()
    # Optional: Add grid to ax1 for better readability
    ax1.grid(True)
    ...
    # CORRECTED: Called on ax2 instead of ax2.xlabel / plt.xlabel
    ax2.set_xlabel('Time (s)')
    ax2.set_ylabel('Shock Pot Length (Inches)')
    ax2.legend()
    # Optional: Add grid to ax2 for better readability
    ax2.grid(True)

    plt.tight_layout(rect=[0, 0, 1, 0.96]) # Adjusted layout to make room for suptitle
    plt.savefig(f'{base_path}_STACKED_PLOT.png')
    plt.close(fig) # Use fig handle to close the whole figure
